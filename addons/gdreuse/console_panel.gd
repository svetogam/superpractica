...
@onready var download_button: Button = %DownloadButton as Button
...
@onready var _normal_color := EditorInterface.get_editor_theme().get_color(
		"font_color", "Editor")
@onready var _success_color := EditorInterface.get_editor_theme().get_color(
		"success_color", "Editor")
@onready var _error_color := EditorInterface.get_editor_theme().get_color(
		"error_color", "Editor")


func _ready() -> void:
	# Set colors
	console.add_theme_color_override("background_color",
		EditorInterface.get_editor_theme().get_color("dark_color_2", "Editor")
	)
...
func show_command_output(p_command: String, output: Array) -> void:
	...
	if command.text.contains("lint"):
		if console.text.contains(":-(") or console.text.contains("Error:"):
			console.add_theme_color_override("font_readonly_color", _error_color)
		elif console.text.contains(":-)"):
