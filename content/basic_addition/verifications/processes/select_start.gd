...
func _init(p_pim: Pim, p_number_effect: NumberEffect) -> void:
	_pim = p_pim
	_number_effect = p_number_effect


func _ready() -> void:
	_start_square = _pim.field.get_number_square(_number_effect.number)
	_number_effect.animator.move_completed.connect(_on_move_completed)
	var dest := _pim.field_viewer.convert_internal_to_external_point(
			_start_square.position)
...
func _on_move_completed() -> void:
	...
	_pim.field.toggle_highlight(_start_square)
