...
const TestField := preload("res://content/basic_addition/pims/grid_counting/field/"
...
func test_query_marked_cells():
	...
	#are_all_marks_contiguous
	field.load_state(CASE_1)
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.ALL)).is_false()
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.UNIT)).is_false()
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.HIGHLIGHT)).is_false()
	...
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.UNIT)).is_false()
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.HIGHLIGHT)).is_true()

	field.load_state(CASE_3)
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.ALL)).is_true()
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.UNIT)).is_false()
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.HIGHLIGHT)).is_false()
	...
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.ALL)).is_true()
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.UNIT)).is_true()
	assert_bool(field.are_all_marks_contiguous(
			GridCounting.CellMarks.HIGHLIGHT)).is_false()
	...
	#get_all_marked_numbers
	field.load_state(EMPTY)
	assert_array(field.get_all_marked_numbers()).is_empty()

	field.load_state(CASE_1)
	assert_array(field.get_all_marked_numbers()).contains_exactly([1, 2, 3, 98, 99, 100])

	field.load_state(CASE_2)
	assert_array(field.get_all_marked_numbers()).contains_exactly(
			[15, 25, 33, 34, 35, 36, 37, 45, 55])

	field.load_state(CASE_3)
	assert_array(field.get_all_marked_numbers()).has_size(20)

	field.load_state(CASE_4)
	assert_array(field.get_all_marked_numbers()).has_size(43)

	#get_marked_numbers
	field.load_state(EMPTY)
	assert_array(field.get_marked_numbers(GridCounting.CellMarks.ALL)).is_empty()
	assert_array(field.get_marked_numbers(GridCounting.CellMarks.UNIT)).is_empty()
	assert_array(field.get_marked_numbers(GridCounting.CellMarks.HIGHLIGHT)).is_empty()

	field.load_state(CASE_1)
	assert_array(field.get_marked_numbers(
			GridCounting.CellMarks.ALL)).contains_exactly([1, 2, 3, 98, 99, 100])
	assert_array(field.get_marked_numbers(
			GridCounting.CellMarks.UNIT)).contains_exactly([1, 2, 3, 98, 99, 100])
	assert_array(field.get_marked_numbers(GridCounting.CellMarks.HIGHLIGHT)).is_empty()

	field.load_state(CASE_2)
	assert_array(field.get_marked_numbers(
			GridCounting.CellMarks.ALL)).contains_exactly(
			[15, 25, 33, 34, 35, 36, 37, 45, 55])
	assert_array(field.get_marked_numbers(
			GridCounting.CellMarks.UNIT)).contains_exactly(
			[15, 25, 33, 34, 36, 37, 45, 55])
	assert_array(field.get_marked_numbers(
			GridCounting.CellMarks.HIGHLIGHT)).contains_exactly([35])

	field.load_state(CASE_4)
	assert_array(field.get_marked_numbers(GridCounting.CellMarks.ALL)).has_size(43)
	assert_array(field.get_marked_numbers(
			GridCounting.CellMarks.UNIT)).contains_exactly([60, 71])
	assert_array(field.get_marked_numbers(
			GridCounting.CellMarks.HIGHLIGHT)).contains_exactly([20])

	#get_smallest_marked_number
	field.load_state(EMPTY)
	assert_int(field.get_smallest_marked_number(
			GridCounting.CellMarks.ALL)).is_equal(-1)
	assert_int(field.get_smallest_marked_number(
			GridCounting.CellMarks.UNIT)).is_equal(-1)
	assert_int(field.get_smallest_marked_number(
			GridCounting.CellMarks.HIGHLIGHT)).is_equal(-1)
	...
	field.load_state(CASE_2)
	assert_int(field.get_smallest_marked_number(
			GridCounting.CellMarks.ALL)).is_equal(15)
	assert_int(field.get_smallest_marked_number(
			GridCounting.CellMarks.UNIT)).is_equal(15)
	assert_int(field.get_smallest_marked_number(
			GridCounting.CellMarks.HIGHLIGHT)).is_equal(35)

	field.load_state(CASE_4)
	assert_int(field.get_smallest_marked_number(
			GridCounting.CellMarks.ALL)).is_equal(1)
	assert_int(field.get_smallest_marked_number(
			GridCounting.CellMarks.UNIT)).is_equal(60)
	assert_int(field.get_smallest_marked_number(
			GridCounting.CellMarks.HIGHLIGHT)).is_equal(20)

	#get_biggest_marked_number
	field.load_state(EMPTY)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.ALL)).is_equal(-1)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.UNIT)).is_equal(-1)
	assert_int(field.get_biggest_marked_number(
	...
	field.load_state(CASE_1)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.ALL)).is_equal(100)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.UNIT)).is_equal(100)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.HIGHLIGHT)).is_equal(-1)

	field.load_state(CASE_2)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.ALL)).is_equal(55)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.UNIT)).is_equal(55)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.HIGHLIGHT)).is_equal(35)

	field.load_state(CASE_4)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.ALL)).is_equal(71)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.UNIT)).is_equal(71)
	assert_int(field.get_biggest_marked_number(
			GridCounting.CellMarks.HIGHLIGHT)).is_equal(20)

	#get_contiguous_number_sequences
	field.load_state(EMPTY)
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.ALL)).is_empty()
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.UNIT)).is_empty()
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.HIGHLIGHT)).is_empty()

	field.load_state(CASE_1)
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.ALL)).contains_exactly([[1, 3], [98, 100]])
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.UNIT)).contains_exactly([[1, 3], [98, 100]])
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.HIGHLIGHT)).is_empty()

	field.load_state(CASE_2)
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.ALL)).contains_exactly([[33, 37]])
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.UNIT)).contains_exactly([[33, 34], [36, 37]])
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.HIGHLIGHT)).is_empty()
	...
	field.load_state(CASE_5)
	assert_array(field.get_contiguous_number_sequences(
			GridCounting.CellMarks.ALL)).contains_exactly([[11, 29]])
	assert_array(field.get_contiguous_number_sequences(
