...
func setup(p_start_number: int, p_addend: int, p_show_count:=true) -> void:
...
func _ready() -> void:
	if _addend_tens_digit > 0:
		field.run_process("circle_numbers_in_direction",
				[_start_number, _addend_tens_digit, "down", _show_count],
				self, "_on_tens_circled")
	elif _addend_ones_digit > 0:
		field.run_process("circle_numbers_in_direction",
				[_second_start_number, _addend_ones_digit, "right", _show_count],
				self, "_on_ones_circled")
	else:
		complete()
