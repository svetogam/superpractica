...
var cells: Array:
	get:
		return [
			...
			field.dynamic_model.get_grid_cell(first_number + 4),
		]
...
func _pressed(_point: Vector2) -> void:
	if $StateChart/States/ActiveModes/Delete/On.active:
		GridCountingActionDeleteFiveBlock.new(field, first_number).push()
...
func _dragged(_external: bool, point: Vector2, _change: Vector2) -> void:
	if $StateChart/States/ActiveModes/Drag/On.active:
		...
		if dest_cell != null:
			drop_action = GridCountingActionMoveFiveBlock.new(
					field, first_number, dest_cell.number - 2)
