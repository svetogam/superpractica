...
class_name GridCountingActionMoveFiftyBlock
extends FieldAction
...
static func get_name() -> int:
	return GridCounting.Actions.MOVE_FIFTY_BLOCK
...
func is_valid() -> bool:
	return (
		from_first_row_number >= 1 and from_first_row_number <= 6
		and to_first_row_number >= 1 and to_first_row_number <= 6
		and from_first_row_number != to_first_row_number
	)


func is_possible() -> bool:
	if field.dynamic_model.get_fifty_block(from_first_row_number) == null:
		return false
	...
	needed_row_numbers.erase(from_first_row_number + 4)
...
func prefigure() -> void:
	if is_valid() and is_possible():
		...
		field.prefigure_fifty_block(from_first_row_number)
