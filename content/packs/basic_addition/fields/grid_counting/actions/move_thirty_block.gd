...
class_name GridCountingActionMoveThirtyBlock
extends FieldAction
...
static func get_name() -> int:
	return GridCounting.Actions.MOVE_THIRTY_BLOCK
...
func is_valid() -> bool:
	return (
		from_first_row_number >= 1 and from_first_row_number <= 8
		and to_first_row_number >= 1 and to_first_row_number <= 8
		and from_first_row_number != to_first_row_number
	)


func is_possible() -> bool:
	if field.dynamic_model.get_thirty_block(from_first_row_number) == null:
		return false
...
func prefigure() -> void:
	if is_valid() and is_possible():
		field.prefigure_thirty_block(to_first_row_number)
	else:
		field.prefigure_thirty_block(from_first_row_number)
...
func do() -> void:
	block = field.dynamic_model.get_thirty_block(from_first_row_number)
