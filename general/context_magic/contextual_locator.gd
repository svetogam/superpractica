...
const SCENE_TREE_SIGNAL_PREFIX := "__registered_for_contextual_locator:"


static func register(context: Node, key: String, value, override_parents := true
) -> void:
...
static func register_property_as(context: Node,
		property_name: String, key: String, override_parents := true
) -> void:
	var meta_key := META_PREFIX + key
...
static func register_getter(getter: Callable, key: String, override_parents := true
) -> void:
	var context = getter.get_object()
	var meta_key := META_PREFIX + key
	context.set_meta(meta_key, {"getter": getter, "override_parents": override_parents})
	_emit_key_registered_signal(context.get_tree(), key)


static func register_child_by_group(context: Node, group: String, override_parents := true
) -> void:
	register_child_by_group_as(context, group, group, override_parents)


static func register_child_by_group_as(context: Node,
		group: String, key: String, override_parents := true
) -> void:
	var meta_key := META_PREFIX + key
...
static func _get_value_by_meta_key(node: Node, meta_key: String):
	...
	elif meta_dict.has("getter"):
		var getter = meta_dict["getter"]
		return getter.call()
...
var _callable_map := CallableMap.new()
...
func auto_callback(key: String, callable: Callable) -> void:
	var value = find(_agent, key)
	if value != null:
		callable.callv([value])
		auto_found.emit(key, value)
		if not _continuous_update_mode:
			_stop_auto_callbacks(key)

	if value == null or _continuous_update_mode:
		_connect_auto_callback(key, callable)


func _connect_auto_callback(key: String, callable: Callable) -> void:
	_callable_map.add(key, callable, not _continuous_update_mode)
	var signal_name := _initialize_scene_tree_signal_for_key(_agent.get_tree(), key)
...
func _try_auto_callbacks(key: String) -> bool:
	var value = find(_agent, key)
	if value != null:
		_callable_map.call_by_key(key, [value])
		auto_found.emit(key, value)
		return true
	return false
...
func _connect_auto_set(key: String, property_name: String) -> void:
	_callable_map.add(key, _set_agent_property.bind(_agent, property_name),
			not _continuous_update_mode)
	var signal_name := _initialize_scene_tree_signal_for_key(_agent.get_tree(), key)
	if not _agent.get_tree().is_connected(signal_name, _try_auto_callbacks):
		_agent.get_tree().connect(signal_name, _try_auto_callbacks.bind(key))


# This is only to switch the bind order on callables
static func _set_agent_property(value: Variant, agent: Object, property_name: String
) -> void:
	agent.set(property_name, value)
